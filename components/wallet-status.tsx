...
import { SUPPORTED_TOKENS, MONAD_CONFIG } from '@/lib/config'
...
export function WalletStatus() {
  ...
  // 模拟代币余额数据 (在实际项目中应该从链上获取)
  const mockTokenBalances = SUPPORTED_TOKENS.map(token => ({
    ...token,
    balance: BigInt(Math.floor(Math.random() * 1000000000000000000)), // 随机余额
    balanceFormatted: (Math.random() * 1000).toFixed(4)
  }))
  ...
  const calculatePortfolioValue = () => {
    ...
    // 原生代币价值
    if (balance && prices['MON']) {
      const nativeValue = parseFloat(balance) * prices['MON'].price
      totalValue += nativeValue
    }
    
    // ERC20代币价值
    mockTokenBalances.forEach(token => {
  ...
  const viewInExplorer = () => {
    if (address) {
      window.open(`${MONAD_CONFIG.blockExplorerUrl}/address/${address}`, '_blank')
    }
  }
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col items-start">
            <span className="text-xs font-medium">
              {chainId === MONAD_CONFIG.chainId ? MONAD_CONFIG.chainName : 'Monad Testnet'}
            </span>
            ...
            <div className="space-y-2">
              ...
              <div className="text-xs space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span>{chainId === MONAD_CONFIG.chainId ? MONAD_CONFIG.chainName : 'Monad Testnet'}</span>
                </div>
                <div className="flex justify-between items-center">
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-blue-50 p-3 rounded-lg">
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      {mockTokenBalances.filter(t => parseFloat(t.balanceFormatted) > 0).length + 
                       (balance && parseFloat(balance) > 0 ? 1 : 0)} 种代币
                    </div>
                  </div>
                    ...
                    <div className="flex items-center justify-between py-2 border-b">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-gray-500">
                          ${(parseFloat(balance) * (prices['MON']?.price || 0)).toFixed(2)}
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {/* 如果没有代币余额 */}
                  {mockTokenBalances.every(token => parseFloat(token.balanceFormatted) === 0) && 
                   (!balance || parseFloat(balance) === 0) && (
